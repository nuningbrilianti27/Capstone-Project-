...
#Dataset: GSE298765 (sel NSCLC PC9 dengan (shLDOC1) vs sel NSCLC PC9 tanpa (shCtrl)) 
#Platform: Microarray (Affymetrix Human Genome U133 Plus 2.0 Array) 
#Tujuan: Mengidentifikasi Differentially Expressed Genes (DEG) 

#PART A. PENGANAR KONSEP  
#Analisis ekspresi gen bertujuan untuk membandingkan dua kondisi sel pada garis sel PC9 cell line yang berasal dari kanker paru Non-Small Cell Lung Cancer.
 
#menggunakan pendekatan statistik limma (Linear 
Models #for Microarray Data), yang merupakan standar emas untuk data 
microarray. 
...
#GPL570 = Affymetrix Human Genome hgu133plus2.db  
...
gset <- getGEO("GSE298765", GSEMatrix = TRUE, AnnotGPL = TRUE)[[1]] 
...
#karena hasilnya TRUE maka kita ubah ke log
...
grup_depletion <- nama_grup[1] 
grup_expressing  <- nama_grup[2] 
contrast_formula <- paste(grup_depletion, "-", grup_expressing)  
...
library(hgu133plus2.db)
gene_annotation$KEGG_ID <- paste0("hsa:", gene_annotation$ENTREZID)
...
#Cek hasil anotasi 
head (topTableResults[,c("PROBEID", "SYMBOL", "GENENAME", "ENTREZID", "KEGG ID")])
...
umap_input <- umap_input[, colSums(is.na(umap_input)) == 0]
umap_result <- umap(umap_input, n_neighbors = 1)
#DATA TERLALU KECIL, SEHINGGA MENGGUNAKAN PCA
...
plot(
  ...
  xlab = "PC1",
  ylab = "PC2",
  main = "PCA Plot Sampel Berdasarkan Ekspresi Gen"
)
legend(
  "topright",
  legend = levels(gset$group),
...
ggplot(volcano_data, aes(x = logFC, y = -log10(adj.P.Val), color = 
  ...
  theme_minimal() + 
  ggtitle("Volcano Plot DEG GEN LDOC1")
...
deg <- topTableResults
sig_genes <- rownames(deg)

sig_genes <- rownames(deg[deg$adj.P.Val < 0.05, ])
gene_convert <- bitr(sig_genes,
                     fromType = "SYMBOL",
                     toType = "ENTREZID",
                     OrgDb = org.Hs.eg.db)
...
gene_label <- ifelse( 
  ...
  top50$PROBEID,      
  # jika SYMBOL kosong → probe ID 
  top50$SYMBOL        
) 
# jika ada → gene symbol 
...
#DEG SIGNIFIKAN(adjPval<0.05)

deg_sig <- topTableResults[topTableResults$adj.P.Val < 0.05 & abs(topTableResults$logFC) >= 1, ]
entrez_genes <- na.omit(deg_sig$ENTREZID)

#FILTER GEN UPREGULATED DAN DOWNREGULATED
up_genes <- deg_sig[deg_sig$logFC > 0 & deg_sig$adj.P.Val < 0.05, ]

down_genes <- deg_sig[deg_sig$logFC < 0 & deg_sig$adj.P.Val < 0.05, ]

write.csv(up_genes, "Hasil up_genes.csv")
#GO analysys (gen Ontology)
library(clusterProfiler)
library(org.Hs.eg.db)
library(enrichplot)
library(ggplot2)
...
dotplot(ego, showCategory = 20) + ggtitle("GO Biological Process Enrichment (Upregulated Genes)")
...
write.csv(kegg_table, "kegg result pathway table")
write.csv(kegg_result, "kegg result pathway")
dotplot(kegg_result, showCategory = 10) +  ggtitle("KEGG Up Regulated genes")
library(pathview)

library(pathview)

pathview(
  gene.data = entrez_ids,
  pathway.id = "hsa05323",
  species = "hsa"
)
